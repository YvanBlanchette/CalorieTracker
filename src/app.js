...
class App {
  ...
  _removeItem(type, e) {
    if (e.target.classList.contains('delete') || e.target.classList.contains('fa-xmark')) {
      // Add confirmation
      if (confirm('Voulez-vous vraiment supprimer l\'item ?')) {
        // Put the target's card into a item variable
        const item = e.target.closest('.card');
        // Put the item's id into an id variable
        const id = item.getAttribute('data-id');

        // Call public function to remove item from stats
  ...
  _reset() {
    if (confirm('Voulez-vous vraiment remettre à zéro ?')) {
      this._tracker.reset();
